...
from galore_torch import GaLoreAdamW, GaLoreAdamW8bit, GaLoreAdafactor, APOLLO, QAPOLLO
...
def setup_model(args):
    ...
    if args.weight_quant:
        assert args.optimizer.lower() in ['q_galore_adamw8bit', 'q_galore_adamw8bit_per_layer', 'q_apollo', 'q_apollo_per_layer']
...
def setup_optimization(args, model, trainable_params, param_groups, id_galore_params):
    ...
    if args.optimizer.lower() == "adam":
    ...
    elif args.optimizer.lower() == "apollo":
        optimizer = APOLLO(param_groups, lr=args.lr, weight_decay=args.weight_decay)
        
    elif args.optimizer.lower() == "q_apollo":
        optimizer = QAPOLLO(param_groups, lr=args.lr, weight_decay=args.weight_decay, betas=(args.beta1, args.beta2), scale_front=args.scale_front)
        ...
        for p in model.parameters():
            if id(p) in id_galore_params:
                optimizer_dict[p] = QAPOLLO([{'params': [p], 
                    'rank': args.rank, 'update_proj_gap': args.update_proj_gap, 'scale': args.apollo_scale, 'proj_type': args.proj_type,
                    "proj": args.proj,'scale_type': args.scale_type}], lr=args.lr, weight_decay=args.weight_decay)
            else:
                if p.requires_grad:
                    optimizer_dict[p] = bnb.optim.Adam8bit([p], lr=args.lr, weight_decay=args.weight_decay)
